...
library(lme4) # to run effects model
...
#paste0- to add values in two columns
rcbd_dfw
```
...
unbalanced data- some data is missing
...
if interaction are significant then only proceed ahead and display the data
...
residuals: model assumption are based on this
...
rcbd_resid <- augment(rcbd_mod) %>%
  mutate(.studresid=rstudent(rcbd_mod)) #rstudent helps to look for outliers
